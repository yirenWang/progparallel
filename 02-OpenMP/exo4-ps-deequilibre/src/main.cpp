...
double parallele_dynamic(double *A, double *B, unsigned long int size)
{
    double S = 0;

#pragma omp parallel for schedule(dynamic, size / 2) reduction(+ \
                                                               : S)
    for (unsigned long j = 0; j < size; j++)
    {
        if (j < size / 10)
...
double parallele_static(double *A, double *B, unsigned long int size)
{
    double S = 0;
